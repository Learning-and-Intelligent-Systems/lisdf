...
import lisdf.parsing.xml_reflection as xmlr
from lisdf.parsing import urdf
...
class TestURDFParserError(unittest.TestCase):
    ...
    def getParseFuncs(self, cls, xml_string):
        ...
        use_static = lambda: cls.from_xml_string(xml_string)  # noqa: E731
        # Bound method for parsing an object (backwards compatibility)
        use_bound = lambda: cls().parse(xml_string)  # noqa: E731
...
<robot name="test">
    <joint name="bad_joint" type="badtype">
        ...
        for func in funcs:
            self.assertParseErrorPath(
                "/robot[@name='test']/joint[@name='bad_joint']", func
            )
...
<robot name="test">
    ...
    def test_unset_required_name_aggregate_ducktype(self):
        """
        If an aggregate duck-typed element does not have a required attribute,
        ensure it is reported with the index
        """
        xml_string = """<?xml version="1.0"?>
<robot name="test">
...
<robot name="test">
    <link name="b">
        ...
        for func in funcs:
            self.assertParseErrorPath(
                "/robot[@name='test']/link[@name='b']/inertial/origin[@xyz]", func
            )
  ...
  <transmission name="simple_trans_bad">
    ...
    <joint name_BAD="foo_joint">
      <hardwareInterface>EffortJointInterface</hardwareInterface>
        ...
        for func in funcs:
            self.assertParseErrorPath(
                "/robot[@name='test']/transmission[@name='simple_trans_bad']", func
            )
