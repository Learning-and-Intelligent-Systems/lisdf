...
from tests.test_parsing.xml_matching import xml_matches
...
class TestURDFParser(unittest.TestCase):
    @mock.patch(
        "lisdf.parsing.xml_reflection.on_error", mock.Mock(side_effect=ParseException)
    )
    def parse(self, xml):
...
<robot name="test" version="1.0">
  ...
  <link name="link2"/>
    ...
    def test_multi_visual_access(self):
        ...
        self.assertEqual(id(robot.links[0].visuals[0]), id(robot.links[0].visual))
    ...
    def test_multi_collision_access(self):
        ...
        self.assertEqual(id(robot.links[0].collisions[0]), id(robot.links[0].collision))
