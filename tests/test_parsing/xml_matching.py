...
def xml_matches(a, b, ignore_nodes=[]):
    if isinstance(a, str):
        return xml_matches(
            xml.dom.minidom.parseString(a).documentElement, b, ignore_nodes
        )
    if isinstance(b, str):
        return xml_matches(
            a, xml.dom.minidom.parseString(b).documentElement, ignore_nodes
        )
