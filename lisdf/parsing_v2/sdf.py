...
class SDFVisitor(XMLVisitor):
    ...
    def geometry(self, node):
        ...
        c = node.children.pop()
        if c.tag == 'box':
            return node.set_data(C.BoxShapeInfo(vector3f(self._pop_children(c, 'size', required=True))))
        elif c.tag == 'sphere':
            return node.set_data(C.SphereShapeInfo(float(self._pop_children(c, 'radius', required=True))))
        elif c.tag == 'cylinder':
            return node.set_data(C.CylinderShapeInfo(
                float(self._pop_children(c, 'radius', required=True)),
                float(self._pop_children(c, 'length', required=True)) / 2,
            ))
        elif c.tag == 'capsule':
            return node.set_data(C.CapsuleShapeInfo(
                float(self._pop_children(c, 'radius', required=True)),
                float(self._pop_children(c, 'length', required=True)) / 2,
            ))
        elif c.tag == 'plane':
            size = vector2f(self._pop_children(c, 'size', required=True))
            return node.set_data(C.PlaneShapeInfo(
                half_width=size[0] / 2, half_height=size[1] / 2,
                normal=vector3f(self._pop_children(c, 'normal', default='0 0 1')),
            ))
        elif c.tag == 'mesh':
            return node.set_data(C.MeshShapeInfo(
    ...
    def material(self, node):
        ...
        return node.set_data(C.PhongMaterial(
    ...
    def joint(self, node):
        ...
        if type == 'fixed':
            ...
            joint_info = C.HingeJointInfo(
                vector3f(self._pop_children(
                    self._pop_children(node, 'axis', required=True, return_type='node'),
                    'xyz', required=True, return_type='text'
                ))
            )
        elif type == 'revolute':
            # TODO: Handle limits
